...
const HERO_UPLOAD_BASE = "/img-upload/admin/heroimg";
const HERO_EMPTY_TOKEN = "__EMPTY__";
...
const normalizeHero = (hero = {}) => {
  ...
  return {
    ...
    heroheader: hero?.heroheader === HERO_EMPTY_TOKEN ? "" : hero?.heroheader ?? "",
    herotitle1: hero?.herotitle1 === HERO_EMPTY_TOKEN ? "" : hero?.herotitle1 ?? "",
    herotitle2: hero?.herotitle2 === HERO_EMPTY_TOKEN ? "" : hero?.herotitle2 ?? "",
    herotitle3: hero?.herotitle3 === HERO_EMPTY_TOKEN ? "" : hero?.herotitle3 ?? "",
    targeturl: hero?.targeturl === HERO_EMPTY_TOKEN ? "" : hero?.targeturl ?? "",
...
const buildPayload = (hero) => {
  ...
  const withToken = (value) => {
    ...
    return cleaned === "" ? HERO_EMPTY_TOKEN : cleaned;
  };

  return {
    heroHeader: withToken(hero?.heroheader ?? hero?.heroHeader),
    heroTitle1: withToken(hero?.herotitle1 ?? hero?.heroTitle1),
    heroTitle2: withToken(hero?.herotitle2 ?? hero?.heroTitle2),
    heroTitle3: withToken(hero?.herotitle3 ?? hero?.heroTitle3),
    targetUrl: withToken(hero?.targeturl ?? hero?.targetUrl),
  };
};
