        ...
        async ({
            ...
            try {
                ...
                if (!hideUnavailable) {
                    ...
                    setTotal(firstPage.total ?? 0);

                    if (updateFilters) {
                        // Build filter options from all items, not just the current page.
                        const totalPagesForFilters = firstPage.totalPages || 1;
                        const allItems = [...(firstPage.items || [])];

                        for (let current = 1; current <= totalPagesForFilters; current += 1) {
                            if (current === (firstPage.page ?? nextPage)) continue;
                            const res = await fetchProducts({ ...baseParams, page: current });
                            allItems.push(...(res.items || []));
                        }

                        deriveFilters(allItems);
                    }
                } else {
